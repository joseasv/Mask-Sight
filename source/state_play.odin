...
state_play_on_enter :: proc() {
	...
	fmt.println("iniciando nivel con textfadetimer ", g.textFadeTimer)
}
...
state_play_update :: proc() {
	...
	if g.fade_phase != 0 {
		...
		if g.textFadeTimer < g.textFadeInTime + g.textFadeHoldTime + g.textFadeOutTime {
			fmt.println("mostrando texto de inicio")

			g.textFadeTimer += dt

			switch {
			case g.textFadeTimer < g.textFadeInTime:
				fmt.println("fade in")
				g.textFadeAlpha = g.textFadeTimer / g.textFadeInTime

			case g.textFadeTimer < g.textFadeInTime + g.textFadeHoldTime:
				fmt.println("fade hold")
				g.textFadeAlpha = 1.0

			case g.textFadeTimer < g.textFadeInTime + g.textFadeHoldTime + g.textFadeOutTime:
				fmt.println("fade out")
				fadeOutElapsed := g.textFadeTimer - g.textFadeInTime - g.textFadeHoldTime
				g.textFadeAlpha = 1.0 - (fadeOutElapsed / g.textFadeOutTime)
...
state_play_draw :: proc() {
	...
	if g.textFadeTimer < g.textFadeInTime + g.textFadeHoldTime + g.textFadeOutTime {
		...
		/*if g.reached_exit {
			texto: cstring = "¡Salida encontrada!"

			rl.DrawText(
				texto,
				i32(g.personaje.pos.x) - i32(rl.MeasureText(texto, 64) / 2),
				i32(g.personaje.pos.y),
				64,
				rl.Fade(rl.GREEN, g.textFadeAlpha),
			)
		} else {
			texto := fmt.ctprintf("Piso %d", g.current_level)
